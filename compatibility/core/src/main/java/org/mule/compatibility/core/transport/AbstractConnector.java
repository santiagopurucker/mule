...
public abstract class AbstractConnector extends AbstractAnnotatedObject implements Connector, WorkListener {
  ...
  protected void initWorkManagers() throws MuleException {
    if (receiverWorkManager.get() == null) {
      ...
      WorkManager newWorkManager =
          this.getReceiverThreadingProfile().createWorkManager(threadPrefix, muleContext.getConfiguration().getShutdownTimeout());
    ...
    if (dispatcherWorkManager.get() == null) {
      ...
      WorkManager newWorkManager =
          dispatcherThreadingProfile.createWorkManager(threadPrefix, muleContext.getConfiguration().getShutdownTimeout());
  ...
  protected MessageDispatcher borrowDispatcher(OutboundEndpoint endpoint) throws MuleException {
    ...
    if (!supportsProtocol(endpoint.getConnector().getProtocol())) {
      throw new IllegalArgumentException(TransportCoreMessages
          .connectorSchemeIncompatibleWithEndpointScheme(this.getProtocol(), endpoint.getEndpointURI().toString()).getMessage());
    }
  ...
  protected Object getReceiverKey(FlowConstruct flowConstruct, InboundEndpoint endpoint) {
    return StringUtils.defaultIfEmpty(endpoint.getEndpointURI().getFilterAddress(), endpoint.getEndpointURI().getAddress());
  }
   ...
   *
   * @param requesterThreadingProfile Value to set for property 'requesterThreadingProfile'.
   */
  ...
  /**
   * @param createMultipleTransactedReceivers if true, multiple receivers will be created for this connection
   * @see #isCreateMultipleTransactedReceivers()
   */
  ...
  /**
   * Returns the number of concurrent receivers that will be launched when {@link #isCreateMultipleTransactedReceivers()} returns
   * <code>true</code>.
   *
  ...
  protected void updateCachedNotificationHandler() {
    if (null != muleContext) {
      notificationHelper =
          new NotificationHelper(muleContext.getNotificationManager(), EndpointMessageNotification.class, dynamicNotification);
    }
  }
   ...
   *
   * Transactional resource factory must be provided by overriding method getOperationResourceFactory().
   *
   * Transactional resource instantiation must be provided by overriding method createOperationResource().
   *
  ...
  /**
   * This method will return the requester to the pool or, if the payload is an inputstream, replace the payload with a new
   * DelegatingInputStream which returns the requester to the pool when the stream is closed.
   *
   * @param endpoint
   * @param requester
   * @param result
  ...
  /**
   * Allows a maxWait timeout to be configured on the dispatcher object pool See: {@link GenericKeyedObjectPool#setMaxWait(long)}
   */
  ...
  /**
   * Allows a maxWait timeout to be configured on the requester object pool See: {@link GenericKeyedObjectPool#setMaxWait(long)}
   */
